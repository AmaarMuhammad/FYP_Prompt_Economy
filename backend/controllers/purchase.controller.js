...
exports.initiatePurchase = async (req, res) => {
  try {
    ...
    // Calculate fees (5% platform fee, 95% to creator)
    const priceNum = BigInt(price);
    const platformFee = (priceNum * 5n) / 100n;
    const creatorEarning = priceNum - platformFee;

    // Create purchase record
    const purchase = await Purchase.create({
      buyer: req.user._id,
      buyerWallet: req.user.walletAddress || 'demo-wallet-' + req.user._id,
      prompt: promptId,
      price,
      platformFee: platformFee.toString(),
      creatorEarning: creatorEarning.toString(),
      transactionHash,
      status: 'pending'
    });
...
exports.verifyPurchase = async (req, res) => {
  try {
    ...
    // Check if this is a demo transaction (skip blockchain verification)
    const isDemoTransaction = purchase.transactionHash.startsWith('demo-tx-');
    
    if (isDemoTransaction) {
      // Demo mode - grant access immediately without blockchain verification
      purchase.blockchainVerified = false;
      purchase.status = 'completed';
      purchase.accessGranted = true;
      purchase.accessGrantedAt = new Date();
      await purchase.save();

      console.log('Purchase verified (demo mode):', {
      ...
      // Update user's totalPurchases counter
