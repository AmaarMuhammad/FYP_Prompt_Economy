...
const UploadPrompt = () => {
  ...
  const { user } = useAuth();
  const { createPrompt, updatePrompt, getPromptById, loading } = useMarketplace();
  const isEditMode = !!id;
  ...
  const loadPromptData = async () => {
    ...
    if (prompt) {
      ...
      if (prompt.creator?._id !== (user?.id || user?._id)) {
      ...
      setFormData({
        ...
        priceInMatic: prompt.price ? (parseFloat(prompt.price) / 1e18).toString() : '',
  ...
  const validateForm = () => {
    ...
    if (!formData.priceInMatic || parseFloat(formData.priceInMatic) < 0) {
      newErrors.priceInMatic = 'Please enter a valid price (0 or greater)';
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode) {
      ...
      result = await updatePrompt(id, promptData);
    } else {
      // Create new prompt
    ...
    if (result) {
      navigate(isEditMode ? `/prompts/${id}` : '/marketplace');
    }
  };
  ...
  return (
    <div className="upload-container">
      <div className="upload-header">
        <h1>{isEditMode ? 'Edit Your Prompt' : 'Upload Your Prompt'}</h1>
        <p>{isEditMode ? 'Update your prompt details' : 'Share your AI prompts with the community and earn MATIC'}</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="upload-form">
        ...
        <div className="form-actions">
          ...
          <button
            ...
            disabled={loading}
          >
            {loading ? (isEditMode ? 'Updating...' : 'Uploading...') : (isEditMode ? 'Update Prompt' : 'Upload Prompt')}
          </button>
        </div>
      </form>
    </div>
